...
export const DeleteItemModal: React.FC<DeleteItemModalProps> = ({ isOpen, onClose, onConfirm, item }) => {
            ...
            >
                ...
                <div className="px-8 pt-8 pb-4 flex items-center justify-between">
                    <h3 className="text-xl font-black text-slate-900 dark:text-white">
                        {t('dashboard.menuEditor.deleteItemModal.title')}
                    </h3>
                    <button
                        onClick={onClose}
                ...
                <div className="px-8 pb-8">
                    <div className="mb-8">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 text-sm font-medium leading-relaxed mb-4">
                            {t('dashboard.menuEditor.deleteItemModal.desc')}
                        </p>
                    ...
                    <div className="flex gap-3">
                        ...
                        >
                            {t('dashboard.menuEditor.deleteItemModal.cancel')}
                        </button>
                        <button
                            onClick={() => {
                                onConfirm();
                            ...
                            className="flex-1 bg-red-500 hover:bg-red-600 text-white font-black py-4 rounded-2xl shadow-lg shadow-red-500/20 transition-all transform active:scale-[0.98] flex items-center justify-center gap-2 text-sm uppercase tracking-widest"
                        >
                            <Trash2 className="w-5 h-5" />
                            <span>{t('dashboard.menuEditor.deleteItemModal.confirm')}</span>
                        </button>
                    </div>
                </div>
            </div>
