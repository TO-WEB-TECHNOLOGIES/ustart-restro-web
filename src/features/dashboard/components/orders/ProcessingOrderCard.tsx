...
import { Clock, UtensilsCrossed, Phone, ChevronDown, Loader2, AlertCircle, Bike, MapPin, Printer, Navigation } from 'lucide-react';
...
export const ProcessingOrderCard = ({ order, onAction, onExtendTime, onCancel, onShowMore }: ProcessingOrderCardProps) => {
    ...
    const isCompleted = order.status === 'DELIVERED' || order.status === 'CANCELLED_BY_CUSTOMER' || order.status === 'CANCELLED_BY_RESTRO';
    ...
    return (
        <div className={`bg-white dark:bg-slate-900 rounded-2xl border ${isUrgent ? 'border-red-500 animate-urgent' : 'border-slate-100 dark:border-slate-800'} shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col md:flex-row min-h-[300px]`}>
                ...
                <div className="mb-4">
                    <div className="flex flex-wrap items-center gap-2 mb-3">
                        ...
                        <div className="flex items-center gap-1.5 w-full">
                            <button className="flex-1 flex items-center justify-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-[10px] font-black uppercase tracking-wider transition-colors border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="flex items-baseline justify-between gap-2">
                        <div>
                            <p className="text-[9px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-0.5">{t('dashboard.orders.card.totalAmount')}</p>
                            <p className="text-xl font-black text-slate-900 dark:text-white tracking-tighter tabular-nums leading-none">₹{order.amount.toFixed(2)}</p>
                        </div>
                        <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md border ${order.paymentMethod === 'PAID'
                    ...
                    {!showTimeline ? (
                        <div className="flex flex-col gap-3">
                            <div>
                                ...
                                <p className="text-xs font-bold text-slate-500 dark:text-slate-400 mt-0.5 font-mono">{order.customer.phone}</p>
                            </div>

                            <div className="flex items-center gap-2 flex-wrap">
                                <div className="flex items-center gap-1.5 max-w-[70%]">
                                    <MapPin className="w-3.5 h-3.5 text-slate-400 flex-shrink-0" />
                                    <p className="text-[11px] font-medium text-slate-600 dark:text-slate-400 leading-none truncate">
                        ...
                        <div className="flex flex-col gap-3 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-700">
                ...
                <div className="h-[200px] space-y-3 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-800 scrollbar-track-transparent pr-2">
                    {displayedItems.map((item, idx) => (
                        <div key={idx} className="flex justify-between items-start group border-b border-slate-50 dark:border-slate-800/50 pb-2 last:border-0 last:pb-0">
                            ...
                            <span className="font-bold text-slate-900 dark:text-white text-sm font-mono">₹{item.price * item.quantity}</span>
                        </div>
                    ))}
                </div>
            ...
            {/* 3rd Column: Delivery & Actions (30% width) */}
            <div className={`w-full md:w-[30%] ${isUrgent ? 'bg-red-50/10 dark:bg-red-900/5' : 'bg-slate-50/30 dark:bg-slate-800/10'} p-5 flex flex-col justify-between`}>
                {/* 3.1 Delivery Partner Details */}
                <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl p-3 shadow-sm mb-4">
                        ...
                        <div className="flex items-center gap-2.5">
                            ...
                            <div>
                                ...
                                <p className="text-md font-black text-slate-900 dark:text-white leading-none">
                                    {t('dashboard.orders.card.deliveryPartner.assigningSoon')}
                                </p>
                            </div>
                        </div>
                    ) : (
                ...
                <div className="space-y-3 mt-auto">
                    {!isCompleted && (
                        <>
                            ...
                            <button
                                ...
                                className={`group relative w-full overflow-hidden py-3.5 rounded-xl font-black text-xs uppercase tracking-[0.2em] transition-all active:scale-[0.98] shadow-lg flex items-center justify-center gap-3 ${isReady
                            ...
                            >
                                ...
                                <span className="relative z-10 flex items-center gap-2">
                                    {isProcessing ? (
                                        ...
                                        <>
                                            <span className='text-md'>{isReady ? t('dashboard.orders.card.complete') : t('dashboard.orders.card.markReady')}</span>
                                            {!isReady && !isDelayed && (
                    ...
                    {isCompleted && (
                        <div className="flex flex-col items-center gap-2 p-3 bg-slate-100 dark:bg-slate-800 rounded-lg">
                            ...
                            <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">
                                {order.status === 'DELIVERED' ? t('dashboard.orders.statuses.delivered') : t('dashboard.orders.statuses.cancelled')}
                            </span>
                        </div>
                    )}
                </div>
