// No Being Used, Just a design reference
...
export const ProcessingOrderCard = ({
  ...
  // Timer Logic
  const approvedLog = order.logs?.find(
    (log) => log.status === "ORDER_APPROVED_BY_RESTRO",
  );
        ...
        <div className="bg-background-white dark:bg-zinc-100 text-slate-800 rounded-t-sm p-5 pb-8 relative z-10">
          ...
          <div className="flex justify-between items-start mb-4 border-b-2 border-dashed border-slate-300 pb-3">
            <div className="flex items-center gap-2 text-slate-700 font-bold text-xs uppercase tracking-wide">
              ...
              <span>
                {isReady
                  ? t("dashboard.recentOrders.statuses.ready")
                  : t("dashboard.recentOrders.statuses.cooking")}
              </span>
            </div>

            {!isReady && (
              <div
                className={`flex items-center gap-1.5 px-2 py-1 rounded text-[10px] font-black uppercase ${
                  isDelayed
              ...
              >
                {isDelayed ? (
                  ...
                  <>
                    <Clock
                      className={`w-3 h-3 ${isUrgent ? "animate-spin-slow" : ""}`}
                    />
                    <span>
                      {t("dashboard.orders.card.minsShort", {
                        count: Math.ceil(timeLeft / 60),
                      })}
                    </span>
                  </>
                )}
          ...
          <div className="text-center mb-5">
            ...
            <p className="font-black text-slate-700 text-sm uppercase truncate px-2 leading-tight">
              {order.customer.name}
            </p>
            ...
            {selectedAddressId === ALL_LOCATIONS_ID &&
          ...
          <div className="space-y-3 mb-6 min-h-[140px] max-h-[30vh] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-800 scrollbar-track-transparent">
            {displayedItems.map((item, idx) => (
              <div
                key={idx}
                className="border-b border-slate-100 border-dotted pb-2 last:border-0 last:pb-0"
              >
                <div className="flex justify-between items-start text-xs mb-0.5">
                  <div className="flex gap-2 items-center flex-1 pr-2">
                    ...
                    <span className="font-black bg-slate-200 text-slate-800 px-1 py-0.5 rounded-sm min-w-[20px] text-center text-[10px]">
                      {item.quantity}
                    </span>
                ...
                {item.description && (
                  <p className="text-[10px] text-slate-500 leading-tight pl-10 font-medium whitespace-normal break-words">
                    {item.description}
                  </p>
                )}
              </div>
            ...
            {order.items.length > 4 && (
              ...
              >
                <>
                  {" "}
                  {t("dashboard.orders.card.moreItems", {
                    count: order.items.length - 4,
                  })}{" "}
          ...
          <div className="mb-6 bg-slate-50 dark:bg-slate-900/50 rounded-xl p-3 border border-slate-100 dark:border-slate-800">
            {order.status === "ORDER_APPROVED_BY_RESTRO" &&
            !order.logs?.some(
              (l) => l.status === "DELIVERY_PARTNER_ASSIGNED",
            ) ? (
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  {(order.status === "DELIVERY_PARTNER_AT_RESTRO" ||
                    order.logs?.some(
                      (l) => l.status === "DELIVERY_PARTNER_AT_RESTRO",
                    )) && (
            ...
            <div className="flex gap-2 mb-3">
                ...
                >
                  {[5, 10, 15, 20].map((mins) => (
                    <option key={mins} value={mins}>
                      {t("dashboard.orders.card.minsPlus", { count: mins })}
                    </option>
                  ))}
                </select>
              ...
              >
                {isExtending ? (
                  ...
                  t("dashboard.orders.card.extendTime")
                )}
              </button>
            </div>
          )}
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className={`group relative w-full overflow-hidden py-3 rounded-md font-black text-xs uppercase tracking-[0.2em] transition-all hover:translate-y-0.5 active:translate-y-1 ${
                isReady
              ...
              <span className="relative z-10 flex items-center justify-center gap-2">
                {isProcessing ? (
                  ...
                  <>
                    <span>
                      {isReady
                        ? t("dashboard.orders.card.complete")
                        : t("dashboard.orders.card.markReady")}
                    </span>
                    {!isReady && !isDelayed && (
